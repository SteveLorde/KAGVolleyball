...
const f32 DEFAULT_THROW_VEL = 7.0f;
...
void onCommand(CBlob@ this, u8 cmd, CBitStream @params)
{
	...
	else if (cmd == this.getCommandID("throw"))
	{
		...
		if (carried !is null)
		{
			if (getNet().isServer() && carried.getName() == "beachball")
			{
				CMap@ map = getMap();
				if (map is null) return;
				f32 mapmid = (map.tilemapwidth * map.tilesize)/2;
				f32 bposx = this.getPosition().x;
				if (bposx < mapmid-296 || bposx > mapmid+296)
				DoThrow(this, carried, pos, Vec2f(0,-1), Vec2f(vel.x,-5));
			}			
			else if (getNet().isServer() && !carried.hasTag("custom throw"))
			{
